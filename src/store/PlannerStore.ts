...
export class PlannerStore {
  ...
  private buildLegacyOverviewHistoryConfigs(rawConfigs: unknown[]): Config[] {
    const legacy = rawConfigs.find(
      (entry: any) =>
        entry?.id === LEGACY_OVERVIEW_HISTORY_ID || entry?.key === LEGACY_OVERVIEW_HISTORY_ID
    ) as any;
    ...
    return [new Config({ key: OVERVIEW_HISTORY_PER_PAGE_KEY, value: perPage })];
  }
  ...
  async loadFromDB() {
    ...
    const deprecatedKeysPresent = DEPRECATED_OVERVIEW_HISTORY_FILTER_KEYS.filter((key) =>
      mergedConfigs.some((config) => config.key === key)
    );
